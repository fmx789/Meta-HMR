...
class CheckpointSaver():
    ...
    def save_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size, dataset_perm, total_step_count,
                        is_update=True, eval_stats=None):
        """Save checkpoint."""
        timestamp = datetime.datetime.now()
        # checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, timestamp.strftime('%Y_%m_%d-%H_%M_%S') + '.pt'))
        if is_update:
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'current' + '.pt'))
        elif eval_stats is None:
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'previous' + '.pt'))
        else:
            # checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'previous_{}_{:.1f}_{:.1f}'.format(epoch, eval_stats['mpjpe'], eval_stats['pa_mpjpe']) + '.pt'))
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir,
                                                               'previous_{}_3dpw_mp{:.1f}_pa{:.1f}_pve{:.1f}_h36m_mp{:.1f}_pa{:.1f}_pve{:.1f}'.format(
                                                                   epoch,
                                                                   eval_stats['3dpw1'],
                                                                   eval_stats['3dpw2'], eval_stats['3dpw3'],
                                                                   eval_stats['h36m1'],
                                                                   eval_stats['h36m2'],
                                                                   eval_stats['h36m3']) + '.pt'))
        checkpoint = {}
    ...
    def get_latest_checkpoint(self):
        ...
        for dirpath, dirnames, filenames in os.walk(self.save_dir):
            for filename in filenames:
                ...
                if filename == 'current.pt':
                    latest_checkpoint = os.path.abspath(os.path.join(dirpath, filename))

...
parser.add_argument('--test_val_step', type=int, default=7, help='# of inner step')
...
parser.add_argument('--save_obj', default=False, action='store_true', help='saving obj')
...
def run_evaluation(model, model_aux, dataset_name, dataset, result_file,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='Eval', total=len(data_loader))):
        ...
        for in_step in range(options.test_val_step + 1):
            ...
            pred_cam_full_inner = cam_crop2full(pred_camera_inner, center, scale, full_img_shape,
                                                focal_length).to(torch.float32)
            
            loss_keypoints = compute_keypoints2d_loss_cliff(
                pred_joints_inner,
                pred_cam_full_inner,
                gt_keypoints_2d.clone(),
                camera_center,
                focal_length,
                crop_trans,
                crop_h,
                crop_w,
                options.use_openpose2d)
        ...
        if viz:
            color_dict = {
                'red': (0.0, 0.4367, 0.898, 1.0),
                'purple1': (0.5137, 0.4353, 1.0, 1.0),#836FFF
                'blue1': (0.5418, 1., 1., 1.0), #8AFFFF
                'pink1': (0.9529, 0.5059, 0.5059, 1.0),
                'purple2': (0.7432, 0.1859, 0.7804, 1.0), #C62FC7
                'pink2': (1.0, 0.5098, 0.7961, 0.6706), #FF82AB
                'blue2': (0.3882, 0.5804, 0.9255, 1.0),
                'brown': (0.8235, 0.70589, 0.5490), #D2B48C
                'orange': (1.0, 0.2705, 0.0, 1.0), #FF4500
                'yellow': (1.0, 0.95, 0.0, 1.0), #FFF200
                }
            ...
            full_img_shape = torch.cat((img_h, img_w), -1)
            ...
            if torch3d == False:
                for idx in range(1):
                    ...
                    if not options.use_openpose2d:
                        if options.save_obj:
                            mesh_filename = 'eval_result/img_viz/' + name + '_ours_gt.obj'
                            images_pred = renderer.visualize_tb(pred_vertices[idx].unsqueeze(0),
                                                                gt_vertices[idx].unsqueeze(0),
                                                                pred_cam_full[idx].unsqueeze(0), image_full, grid=False,
                                                                baseColorFactor=colorsys.hsv_to_rgb(0.0, 0.4367, 0.898),
                                                                blank_bg=True, mesh_filename=mesh_filename)
                        else:
                            images_pred = renderer.visualize_tb(pred_vertices[idx].unsqueeze(0), gt_vertices[idx].unsqueeze(0),
                                                                ...
                                                                blank_bg=True)
                        cv2.imwrite(
                            'eval_result/img_viz/' + name + '_ours_gt.jpg',
                            np.ascontiguousarray(images_pred * 255.0).transpose(1, 2, 0)[:, :, ::-1])

                    else:
                        if options.save_obj:
                            mesh_filename = 'eval_result/img_viz/' + name + '_ours_openpose.obj'
                            images_pred = renderer.visualize_tb(pred_vertices[idx].unsqueeze(0),
                                                                gt_vertices[idx].unsqueeze(0),
                                                                pred_cam_full[idx].unsqueeze(0), image_full, grid=False,
                                                                baseColorFactor=colorsys.hsv_to_rgb(0.9518, 0.7793,
                                                                                                    0.8706),
                                                                mesh_filename=mesh_filename)
                        else:
                            images_pred = renderer.visualize_tb(pred_vertices[idx].unsqueeze(0), gt_vertices[idx].unsqueeze(0),
                                                                pred_cam_full[idx].unsqueeze(0), image_full, grid=False,
                                                                baseColorFactor=colorsys.hsv_to_rgb(0.9518, 0.7793, 0.8706))

                        cv2.imwrite(
                            'eval_result/img_viz/' + name + '_ours_openpose.jpg',
                            np.ascontiguousarray(images_pred * 255.0).transpose(1, 2, 0)[:, :, ::-1])

                    cv2.imwrite('eval_result/img_viz/' + name + '_input.jpg',
                                np.ascontiguousarray(rotated_img * 255.0).transpose(1, 2, 0)[:, :, ::-1])
                ...
                for idx in range(1):
                    ...
                    cv2.imwrite('eval_result/img_viz/' + name + '_input.jpg',
                                np.ascontiguousarray(rgb_img)[:, :, ::-1])

                    image_full = torch.from_numpy(rgb_img).to(images.device).unsqueeze(0)

                    if not options.use_openpose2d:
                        images_pred = r3d.render_mesh(pred_vertices[idx].unsqueeze(0), pred_cam_full[idx].unsqueeze(0),
                                                      image_full, base_color=color_dict['blue1'], grid=False,
                                                      blank_bg=False)

                        cv2.imwrite(
                            'eval_result/img_viz/' + name + '_ours_gt.jpg',
                            np.ascontiguousarray(images_pred)[:, :, ::-1])

                    else:
                        images_pred_step = r3d.render_mesh(pred_vertices[idx].unsqueeze(0),
                                                           pred_cam_full[idx].unsqueeze(0),
                                                           image_full, base_color=color_dict['pink2'], grid=False,
                                                           blank_bg=False)
                        cv2.imwrite('eval_result/img_viz/' + name + '_ours_openpose.jpg',
                                    np.ascontiguousarray(images_pred_step)[:, :, ::-1])
...
if __name__ == '__main__':
    ...
    model = build_model(config.SMPL_MEAN_PARAMS, pretrained=False, backbone=args.backbone, name='cliff_meta',
                        bbox_type=args.bbox_type)
    model_aux = build_model(config.SMPL_MEAN_PARAMS, pretrained=False, backbone=args.backbone, name='cliff_aux',
                            bbox_type=args.bbox_type)

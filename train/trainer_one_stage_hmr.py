...
from eval_hmr import run_evaluation
...
class Trainer_hmr(BaseTrainer):
    ...
    def init_fn(self):
       ...
       # train with multiple datasets
        ...
        self.val_steps = self.options.val_steps
        ...
        self.model = build_model(config.SMPL_MEAN_PARAMS, pretrained=True, backbone=self.options.backbone, name='hmr_meta',
                                 bbox_type=self.options.bbox_type).to(self.device)
        self.model_aux = build_model(config.SMPL_MEAN_PARAMS, pretrained=True, backbone=self.options.backbone, name='hmr_aux',
                                     bbox_type=self.options.bbox_type).to(self.device)
    ...
    def keypoint_3d_loss(self, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d, dataset=None, viz=False,
        ...
        if use_model:
            ...
            conf = torch.ones_like(gt_keypoints_3d[:, :, -1].unsqueeze(-1)[:, 25:, :], device=self.device)
    ...
    def train_step(self, input_batch, cur_epoch, cur_step):
        ...
        for i in range(group_num):
            ...
            for in_step in range(self.inner_steps):
                pred_rotmat_inner, pred_betas_inner, pred_camera_inner = self.model(images_tr, params=params_cp)

                pred_rotmat_aux, pred_betas_aux, pred_camera_aux = self.model_aux(images_tr)
                ...
                pred_cam_t = torch.stack([pred_camera_inner[:, 1],
                                          pred_camera_inner[:, 2],
                                          2 * self.render_focal_length / (self.bbox_size * pred_camera_inner[:, 0] + 1e-9)],
                                         dim=-1)

                pred_keypoints_2d_inner = perspective_projection(pred_joints_inner,
                                                           rotation=torch.eye(3, device=self.device).unsqueeze(
                                                               0).expand(group_size, -1, -1),
                                                           translation=pred_cam_t,
                                                           focal_length=self.render_focal_length,
                                                           camera_center=camera_center_tr)
                # Normalize keypoints to [-1,1]
                gt_keypoints2d_norm = gt_keypoints_2d.clone()[task_id]
                pred_keypoints_2d_inner[:, :, 0] = pred_keypoints_2d_inner[:, :, 0] / (self.crop_w / 2.)
                gt_keypoints2d_norm[:, :, 0] = 2. * gt_keypoints2d_norm[:, :, 0] / self.crop_w - 1.
                pred_keypoints_2d_inner[:, :, 1] = pred_keypoints_2d_inner[:, :, 1] / (self.crop_h / 2.)
                gt_keypoints2d_norm[:, :, 1] = 2. * gt_keypoints2d_norm[:, :, 1] / self.crop_h - 1.

                # Compute 2D reprojection loss for the keypoints
                loss_keypoints_inner = self.keypoint_loss(pred_keypoints_2d_inner, gt_keypoints2d_norm,
                                                    self.options.openpose_train_weight,
                                                    self.options.gt_train_weight)
            ...
            # outer loop
            pred_rotmat, pred_betas, pred_camera = self.model(images_tr, params=params_cp)
            ...
            # Normalize keypoints to [-1,1]
            gt_keypoints2d_norm = gt_keypoints_2d.clone()[task_id]
            pred_keypoints_2d[:, :, 0] = pred_keypoints_2d[:, :, 0] / (self.crop_w / 2.)
            gt_keypoints2d_norm[:, :, 0] = 2. * gt_keypoints2d_norm[:, :, 0] / self.crop_w - 1.
            pred_keypoints_2d[:, :, 1] = pred_keypoints_2d[:, :, 1] / (self.crop_h / 2.)
            gt_keypoints2d_norm[:, :, 1] = 2. * gt_keypoints2d_norm[:, :, 1] / self.crop_h - 1.

            # Compute 2D reprojection loss for the keypoints
            loss_keypoints = self.keypoint_loss(pred_keypoints_2d, gt_keypoints2d_norm,
                                                      self.options.openpose_train_weight,
                                                      self.options.gt_train_weight)
    ...
    def test(self, epoch):
        ...
        mpjpe_3dpw, pa_mpjpe_3dpw, pve_3dpw = run_evaluation(self.model, self.model_aux,
                                                             '3dpw', self.eval_dataset_3dpw, None,
                                                             batch_size=self.options.batch_size,
                                                             shuffle=False,
                                                             log_freq=50,
